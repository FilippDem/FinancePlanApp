            ...
            with cashflow_tab1:
                ...
                if st.session_state.selected_cashflow_year:
                    ...
                    if year_data:
                        ...
                        # Sankey Diagram for Money Flow (full width outside columns)
                        st.markdown("---")
                        ...
                        if cashflow_value >= 0:
                            ...
                            if year_data.get('investment_income', 0) > 0:
                                ...
                                investment_income_formatted = f"${year_data['investment_income']:,.0f}"
                                net_change_formatted = f"${net_change:,.0f}"
                                if net_change >= 0:
                                    st.warning(f"⚖️ Investment returns of {investment_income_formatted} cover the deficit with {net_change_formatted} remaining")
                                else:
                                    st.error(f"⚖️ Even with {investment_income_formatted} investment returns, net change is {net_change_formatted}")
